...
def _ensure_local_alias(savedir: str, alias_name: str, source_name: str) -> bool:
    """Create a copy alias if the source exists but the alias is missing.

    Returns True if the alias exists or was created successfully.
    """
    alias_path = os.path.join(savedir, alias_name)
    source_path = os.path.join(savedir, source_name)
    ...
    try:
        ...
        logging.info(
            "[Diarization][SpeechBrain] Created %s from %s to satisfy offline loading",
            alias_name,
            source_name,
        )
        return True
        ...
        logging.warning(
            "[Diarization][SpeechBrain] Failed to create alias %s from %s: %s",
            alias_name,
            source_name,
            exc,
        )
...
def _ensure_custom_stub(savedir: str) -> None:
    """Create a minimal custom.py so SpeechBrain does not try to fetch it."""
    custom_path = os.path.join(savedir, "custom.py")
    ...
    try:
        ...
        logging.info("[Diarization][SpeechBrain] Created stub custom.py for offline loading at %s", custom_path)
    except Exception as exc:
        logging.debug("[Diarization][SpeechBrain] Failed to create stub custom.py: %s", exc)
...
def _extract_embeddings_speechbrain(audio, sr: int, voiced_segments: List[Tuple[float, float]]):
    ...
    alias_files = (("label_encoder.ckpt", "label_encoder.txt"),)
    ...
    if savedir is None:
        ...
        if os.path.isdir(savedir):
            for alias_name, source_name in alias_files:
                _ensure_local_alias(savedir, alias_name, source_name)
            _ensure_custom_stub(savedir)
